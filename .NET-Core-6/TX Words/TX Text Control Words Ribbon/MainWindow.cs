	...
	public partial class MainWindow : RibbonForm {
        ...
        **-----------------------------------------------------------------------------------------------------*/
		private void SetAppMenuBehavior() {
			...
			// Document Settings:
			m_rtbtnDocumentSettings.CheckedChanged += DocumentSettings_CheckedChanged; // Hide/show sidebar when button is toggled        		

			// About:
			m_rtbtnAbout.CheckedChanged += About_CheckedChanged;

			// Exit:
			m_rbtnExit.Click += Exit_Click;
        ...
        **-----------------------------------------------------------------------------------------------------*/
		internal static void Application_ThreadException(object sender, System.Threading.ThreadExceptionEventArgs e) {
			...
			if (e.Exception.GetType() == typeof(TargetInvocationException) &&
				...
				MessageBox.Show(string.Format(Properties.Resources.MessageBox_Application_ThreadException_Text, e.Exception.InnerException.Message), strProductName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}
